...
class mh_magazine_custom_slider extends WP_Widget {
    function __construct() {
		parent::__construct(
			...
			array('classname' => 'mh_magazine_custom_slider', 'description' => esc_html__('MH Custom Slider widget to display custom content for use on homepage template.', 'mh-magazine'))
		);
	}
    function widget($args, $instance) {
		...
		echo $args['before_widget']; ?>
        	<div id="<?php echo esc_attr($args['widget_id']); ?>" class="flexslider mh-slider-widget <?php echo 'mh-slider-' . esc_attr($instance['slider_layout']); ?>">
				<ul class="slides"><?php
    ...
    function update($new_instance, $old_instance) {
		...
		if ('layout1' !== $new_instance['slider_layout']) {
			if (in_array($new_instance['slider_layout'], array('layout2'))) {
